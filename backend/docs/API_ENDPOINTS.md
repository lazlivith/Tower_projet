...
| `/api/upload` | `POST /image` (🔒) · `POST /document`, `POST /video` (👨‍🏫/🛠️) — champ `scope` (blog·projects·services·courses·certificates·invoices·avatars·misc) → dossier dédié ; Cloudinary si configuré, sinon disque local | 🔒 |
